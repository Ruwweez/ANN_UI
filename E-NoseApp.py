...
class ENoseApp:
    ...
    def __init__(self, root):
        ...
        graph_type_frame.pack(fill="x", padx=20, pady=(10, 0))
        ...
        self.graph_type_combo.pack(side="left")
        ...
        profile_frame.pack(fill="x", padx=20, pady=(8, 0))
        ...
        ann_profile_frame.pack(fill="x", padx=20, pady=(8, 0))

        ttk.Label(
            ann_profile_frame,
            text="ANN Training Profile:",
            font=("Arial", 11, "bold")
        ).pack(side="left", padx=(0, 10))
        ...
        self.ann_profile_combo.pack(side="left")
        ...
        initial_status = (
            ...
            else "Status: Stopped (simulated data - no Arduino)"
        )
    ...
    def start_test(self):
        ...
        self.status_label.config(
            text=(
                ...
                if self.hardware_connected
                else "Status: Testing (simulated data - no Arduino)"
            )
        )
    ...
    def read_sensor_values(self):
        ...
        if self.hardware_connected and self.arduino is not None:
            try:
                ...
                #Empty read (timeout) or malformed line - reuse the
                #last good reading rather than plotting a false zero.
                return self._last_sensor_values

            except (serial.SerialException, ValueError, UnicodeDecodeError):
                #Lost connection or garbled data - drop to simulated
                #mode for the rest of the run instead of crashing.
                self.hardware_connected = False
                self.status_label.config(
                    text="Status: Arduino disconnected - using simulated data"
                )

        # Simulated fallback (no hardware, or hardware just dropped out)
    ...
    def draw_sensor_graph(self):
        ...
        self.figure.subplots_adjust(right=0.78, bottom=0.22)

        ax.set_facecolor(BG_COLOR)
        ax.set_title("Sensor Data Over Time", color=FG_COLOR)
        ax.set_xlabel("Time (s)", color=FG_COLOR)
        ax.set_ylabel("Sensor Values", color=FG_COLOR)
    ...
    def draw_sensor_correlation_heatmap(self):
        ...
        ax = self.figure.add_subplot(111)
        self.figure.subplots_adjust(left=0.22, right=0.95, bottom=0.22, top=0.88)

        ax.set_facecolor(BG_COLOR)

        if len(self.time_data) < 2:
            ax.set_title(
                "Sensor Correlation Heatmap", color=FG_COLOR, fontsize=12
            )
            ax.text(
                0.5, 0.5,
                "Not enough data yet -\nstart a test to collect readings.",
                color=FG_COLOR, ha="center", va="center", fontsize=10,
        ...
        ax.set_title(
            "MQ Sensor Correlation Heatmap", color=FG_COLOR, fontsize=12
        )
        ax.set_xticks(range(NUM_SENSORS))
        ax.set_yticks(range(NUM_SENSORS))
        ax.set_xticklabels(SENSOR_NAMES, color=FG_COLOR)
        ax.set_yticklabels(SENSOR_NAMES, color=FG_COLOR)
        ...
        colorbar.ax.tick_params(color=FG_COLOR, labelcolor=FG_COLOR, length=0)
    ...
    #CLEAN EXIT
    #Closes the Arduino serial connection (if open) before the window
    #closes, so the port isn't left locked for the next run.
    def on_close(self):
